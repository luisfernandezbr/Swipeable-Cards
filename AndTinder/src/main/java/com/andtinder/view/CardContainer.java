...
public class CardContainer extends AdapterView<ListAdapter> {
    ...
    //private static final double DISORDERED_MAX_ROTATION_RADIANS = Math.PI / (64 * 4);
    private static final double DISORDERED_MAX_ROTATION_RADIANS = Math.PI / 32;
    ...
    public PercetageObserver getPercentObserver() {
        return mPercentObserver;
    }

    public void setPercentObserver(PercetageObserver percentObserver) {
        this.mPercentObserver = percentObserver;
    }

    private PercetageObserver mPercentObserver;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                Log.i("DRAG_TEST", "x:" + x + " y:" + y);
                //Log.i("DRAG_TEST", "mLastTouchX:" + mLastTouchX + " mLastTouchY:" + mLastTouchY);

                dx = x - mLastTouchX;
                dy = y - mLastTouchY;

                Log.i("DRAG_TEST", "dx:" + dx + " dy:" + dy + "\n");

                Log.i("DRAG_TEST", "mRequestedWidth:" + mRequestedWidth + " mRequestedHeight:" + mRequestedHeight + "\n");
                Log.i("DRAG_TEST", "\n");


                if (mPercentObserver != null) {

                    int screenCenter = mRequestedWidth / 2;

                    //mPercentObserver
                    float percent = -((screenCenter - x) * 100 / screenCenter);

                    if (percent > 0) {
                        mPercentObserver.likePercent((int)percent);
                    } else {
                        mPercentObserver.unlikePercent(-(int) percent);
                    }
                }
                ...
                float rotation = 20 * mTopCard.getTranslationX() / (getWidth() / 2.f);
                mTopCard.setRotation(rotation);

                Log.i("DRAG_TEST_ROTATION", "rotation:" + rotation);

                mTopCard.setAlpha(0.8f);
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                Log.i("DRAG_TEST_ROTATION", "mTopCard.getRotation():" + mTopCard.getRotation() + " mTopCard.getRotationX():" + mTopCard.getRotationX() + "\n");
                ...
                ValueAnimator animator = ObjectAnimator.ofPropertyValuesHolder(mTopCard,
                        ...
                        PropertyValuesHolder.ofFloat("rotation", mTopCard.getRotation() - mTopCard.getRotation()),
    ...
    private class GestureListener extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (Math.abs(dx) > mTouchSlop && Math.abs(velocityX) > Math.abs(velocityY) && Math.abs(velocityX) > mFlingSlop * 3) {
                ...
                left = 0 - topCard.getWidth() - 100;
                top = 0 - topCard.getHeight() - 100;
                right = getWidth() + 100;
                ...
                Log.i("RESULT_TEST", "targetX: " + targetX);
